...
function Money({ value, receipt }: { value: number; receipt: OrderReceiptData }) {
  return (
    <bdi dir="ltr" className="receipt-number">
      {formatMoney(value, receipt.currency, receipt.locale)}
    </bdi>
  );
}

function ReceiptHeader({ receipt }: { receipt: OrderReceiptData }) {
  const { cafe } = receipt;
  return (
    <header className="receipt-header">
      {cafe.logo ? (
        // Tenant logos can be local assets or tenant-hosted URLs, so a plain print-safe image is intentional.
...
function ReceiptOrderInfo({ receipt }: { receipt: OrderReceiptData }) {
  const { order } = receipt;
  const createdAt = new Date(order.createdAt);
  const date = createdAt.toLocaleDateString(receipt.locale, {
    timeZone: receipt.timezone,
    year: "numeric",
    month: "2-digit",
    day: "2-digit",
  });
  const time = createdAt.toLocaleTimeString(receipt.locale, {
    timeZone: receipt.timezone,
    hour: "2-digit",
    minute: "2-digit",
  });
  return (
    <section className="receipt-info" aria-label="معلومات الطلب">
      ...
      {order.type === "TABLE" && order.tableNumber ? (
        <div><span>الطاولة</span><bdi dir="ltr">T-{order.tableNumber}</bdi></div>
      ) : null}
      ...
      {order.status === "CANCELLED" || order.status === "REFUNDED" ? (
        <div className="receipt-state"><span>حالة الطلب</span><strong>{orderStatusPresentation[order.status].label}</strong></div>
      ) : null}
    </section>
  );
}

function ReceiptItems({ receipt }: { receipt: OrderReceiptData }) {
  return (
        ...
        {receipt.items.map((item) => (
          <article className="receipt-item" key={item.id}>
            ...
            {item.modifiers.map((modifier) => (
              <div className="receipt-item-detail" key={modifier.id}>
                <span>+ {modifier.name}</span>
                {modifier.amount ? <Money value={modifier.amount} receipt={receipt} /> : <span>—</span>}
              </div>
            ))}
...
function ReceiptTotals({ receipt }: { receipt: OrderReceiptData }) {
  return (
    <section className="receipt-totals" aria-label="الإجماليات">
      {receipt.totals.map((row) => (
        <div key={row.key}><span>{row.label}</span><Money value={row.amount} receipt={receipt} /></div>
      ))}
...
function ReceiptPayment({ receipt }: { receipt: OrderReceiptData }) {
...
function ReceiptFooter({ receipt }: { receipt: OrderReceiptData }) {
  ...
  return (
    <footer className="receipt-footer">
      <ReceiptSeparator />
      {messages.length ? messages.map((message) => <p key={message}>{message}</p>) : <p>شكراً لزيارتكم</p>}
    </footer>
  );
}
  ...
  return (
    <div className="receipt-preview" data-receipt-preview>
      <article className="thermal-receipt" data-receipt data-order-number={receipt.order.number} dir="rtl">
        ...
        <ReceiptSeparator />
        <ReceiptOrderInfo receipt={receipt} />
        <ReceiptSeparator />
